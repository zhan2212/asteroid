		...
		for (Movable movFriend : CommandCenter.getInstance().getMovFriends()) {
			for (Movable movFoe : CommandCenter.getInstance().getMovFoes()) {
				...
				if (pntFriendCenter.distance(pntFoeCenter) < (nFriendRadiux + nFoeRadiux)) {
					...
					else if(movFriend instanceof Shield || movFriend instanceof ClearAllWeapon){

					}
					else if(movFriend instanceof DebrisWeapon){
						CommandCenter.getInstance().getOpsList().enqueue(movFriend, CollisionOp.Operation.REMOVE);
						// make debris
						for(int i = 0; i < 360; i += 30)
						CommandCenter.getInstance().getOpsList().enqueue(
								new Debris(((DebrisWeapon) movFriend).getDeltaX(),
										((DebrisWeapon) movFriend).getDeltaY(),
										movFriend.getCenter(),
										((DebrisWeapon) movFriend).getOrientation()+i),
					...
					killFoe(movFoe);
					CommandCenter.getInstance().setScore(CommandCenter.getInstance().getScore()+10); /// set score
		...
		if (CommandCenter.getInstance().getFalcon() != null){
			...
			for (Movable movFloater : CommandCenter.getInstance().getMovFloaters()) {
				...
				if (pntFalCenter.distance(pntFloaterCenter) < (nFalRadiux + nFloaterRadiux)) {
					...
					CommandCenter.getInstance().setNumFalcons(
							CommandCenter.getInstance().getNumFalcons() + 1);// increment the number of falcons
	...
	public void keyReleased(KeyEvent e) {
		...
		if (fal != null) {
			...
			case FIRE:
				...
				break;
			case SHIELD:
				CommandCenter.getInstance().getOpsList().enqueue(new Shield(fal,0), CollisionOp.Operation.ADD);
				CommandCenter.getInstance().getOpsList().enqueue(new Shield(fal,180), CollisionOp.Operation.ADD);
				Sound.playSound("laser.wav");
				break;
			case CLEAR:
				CommandCenter.getInstance().getOpsList().enqueue(new ClearAllWeapon(fal), CollisionOp.Operation.ADD);
				//for (Movable movFoe : CommandCenter.getInstance().getMovFoes()) {
				//	CommandCenter.getInstance().getOpsList().enqueue(movFoe, CollisionOp.Operation.REMOVE);
				//}
			...
			case SPECIAL:
				...
				break;
			case DEBRISWEAPON:
				CommandCenter.getInstance().getOpsList().enqueue(new DebrisWeapon(fal), CollisionOp.Operation.ADD);
				//Sound.playSound("laser.wav");

				break;

			case EXPLOSION:
				CommandCenter.getInstance().getOpsList().enqueue(new newWeapon(fal), CollisionOp.Operation.ADD);
				//Sound.playSound("laser.wav");
				break;
